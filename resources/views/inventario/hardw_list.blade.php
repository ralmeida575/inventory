...
@endsection
        ...
        <x-label class="overflow-hidden shadow sm:rounded-lg mt-2 text-gray-600 text-sm p-6 border-t" style="margin-top: 0px;" >
          <h2 class="font-semibold text-xl text-gray-800 leading-tight">
              {{ __('Lista de Hardware') }}
          </h2>
      </x-label><br>
      <form action="{{route('hardw_search')}}" method="POST">
        @csrf
    ...
    <tr> 
      <td><a href="{{"/hardw/item/" . strval($item->id)}}"><img src="/img/ver.png" style=" float:left" title="Visualizar PC {{$item->tag}} De {{$item->proprietario}}"></a>&nbsp;{{$item->tag}}</td>
      <td>{{$item->marca}}</td>
      <td>{{$item->modelo}}</td>
      <td>{{$item->n_serie}}</td>
      <td>{{$item->tipo}}</td>
      <td>{{$item->data_instal}}</td>
      @if($item->status == 'on')<td><img src="/img/on.jpg" style="width:20px;"></td>
      @else<td><img src="off.jpg" style="width:20px;"></td>
      @endif
